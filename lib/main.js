...
module.exports = {
  consumeIdeClient(service) {
      ...
      async resolveServer(context) {
        ...
        if (!launch) {
          if (!missingReported) {
            ...
            lumine.notifications.addError("Unable to find texlab", {
              description:
              ...
              dismissable: true,
              // The moment the user learns it is missing is the moment to offer
              // the fix, rather than sending them off to a shell and back.
              buttons: [
                {
                  text: "Install Texlab",
                  // Progress and failure are reported by the install itself.
                  onDidClick: () => service.installServer("ide-texlab").catch(() => {}),
                },
              ],
            });
          }
        ...
        // Armed again, so removing the server later is reported once more
        // rather than silently leaving the package doing nothing.
    ...
    subscriptions.add(
      lumine.config.onDidChange("ide-texlab.serverPath", () => {
        for (const session of service.getSessions()) {
          ...
          service.restart(session).catch((error) => {
            lumine.notifications.addError("Unable to restart Texlab Language Server", {
